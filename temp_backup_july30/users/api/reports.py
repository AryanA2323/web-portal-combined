...
@router.get(
    ...
    description="Get all AI-generated reports for legal review (CaseManager only)."
)
def list_reports(request: HttpRequest, status: Optional[str] = None):
    """List all reports with optional status filter."""
    user = request.auth

    # Only caseManagers can access this endpoint
...
@router.get(
    ...
    description="Get statistics about reports (CaseManager only)."
)
...
def get_report(request: HttpRequest, report_id: int):
    ...
    # Check access
    if user.role in [CustomUser.Role.CASE_MANAGER, CustomUser.Role.SUPER_ADMIN]:
        pass  # CaseManager can access any report
...
@router.get(
    ...
    description="Get all active qcs for assignment (CaseManager only)."
)
...
@router.post(
    ...
    description="Assign a qc to review a report (CaseManager only)."
)
...
@router.put(
    ...
    description="Update the content of a report (CaseManager only, before assignment or after rejection)."
)
...
@router.delete(
    ...
    description="Delete an AI-generated report. CaseManager access required.",
)
def delete_report(request: HttpRequest, report_id: int):
    """Delete a report by ID."""
    user = request.auth

    # Only caseManagers can delete reports
    if user.role not in [CustomUser.Role.CASE_MANAGER, CustomUser.Role.SUPER_ADMIN]:
        raise HttpError(403, "Access denied")
