...
def _resolve_login_user(identifier: str):
    ...
    We keep this explicit so login continues to work even if a deployment
    is missing a custom authentication backend or if the identifier is
    supplied in email form by a vendor account.
    """
    if not identifier:
        return None
...
def login_view(request, payload: LoginWith2FASchema):
    ...
    """
    # Authenticate user by explicit username/email lookup first so the
    # login flow remains stable across deployments.
    user = _resolve_login_user(payload.username)
    ...
    # Self-heal old ADMIN roles to CASE_MANAGER
    ...
    device_limit = getattr(user, 'device_limit', 1)
    if active_sessions_count >= device_limit:
        return 403, {
...
def _create_role_specific_profile(user):
    ...
    try:
        if user.role == 'CASE_MANAGER':
            # Create/update caseManager profile
            caseManager_profile, created = CaseManager.objects.get_or_create(
                user=user,
                defaults={
            ...
            if created:
                logger.info(f"Created caseManager profile for user: {user.username}")
...
def register_user(request, payload: UserCreateSchema):
    ...
    - **role**: User role (CLIENT, VENDOR, CASE_MANAGER, or QC)
    
    Returns user info and authentication token on success.
    """
    # Validate role
    valid_roles = ['CLIENT', 'VENDOR', 'CASE_MANAGER', 'QC']
