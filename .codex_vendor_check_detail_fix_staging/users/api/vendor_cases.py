...
def get_vendor_ids_from_user(user):
    """Return possible vendor assignment IDs for a user."""
    if not user.is_authenticated or user.role != 'VENDOR':
        return ()
...
def _get_check_detail_meta(table: str) -> Optional[dict]:
    alias_map = {
        'claimant_checks': 'cc',
        'insured_checks': 'ic',
        'driver_checks': 'dc',
    ...
    if table == 'claimant_checks':
        return {
            'select': f'''{alias}.id, {alias}.case_id, {alias}.check_status,
                         {alias}.claimant_name, {alias}.claimant_contact, {alias}.claimant_address,
                         {alias}.claimant_income, {alias}.statement, {alias}.triggers, {alias}.vendor_evidence AS evidence,
                         {alias}.vendor_documents AS vendor_documents, {alias}.case_documents AS case_documents,
                         {_first_existing_select_expr(table, alias, ['case_manager_feedback', 'admin_feedback'])} AS case_manager_feedback,
    ...
    elif table == 'insured_checks':
        return {
            'select': f'''{alias}.id, {alias}.case_id, {alias}.check_status,
                         {alias}.insured_name, {alias}.insured_contact, {alias}.insured_address,
                         {alias}.policy_number, {alias}.policy_period, {alias}.rc, {alias}.permit,
                         {alias}.statement, {alias}.triggers, {alias}.vendor_evidence AS evidence,
                         {alias}.vendor_documents AS vendor_documents, {alias}.case_documents AS case_documents,
                         {_first_existing_select_expr(table, alias, ['case_manager_feedback', 'admin_feedback'])} AS case_manager_feedback,
                         {_select_expr(table, alias, 'is_reassigned', 'FALSE')} AS is_reassigned,
                         {_select_expr(table, alias, 'questionnaire', "'{{}}'::jsonb")} AS questionnaire,
    ...
    elif table == 'driver_checks':
        return {
            'select': f'''{alias}.id, {alias}.case_id, {alias}.check_status,
                         {alias}.driver_name, {alias}.driver_contact, {alias}.driver_address,
                         {alias}.dl, {alias}.permit, {alias}.occupation,
                         {alias}.statement, {alias}.triggers, {alias}.vendor_evidence AS evidence,
                         {alias}.vendor_documents AS vendor_documents, {alias}.case_documents AS case_documents,
                         {_first_existing_select_expr(table, alias, ['case_manager_feedback', 'admin_feedback'])} AS case_manager_feedback,
                         {_select_expr(table, alias, 'is_reassigned', 'FALSE')} AS is_reassigned,
            ...
            'fields': ['id','case_id','check_status','driver_name','driver_contact',
                        ...
                        'statement','triggers','evidence','vendor_documents','case_documents',
                        'case_manager_feedback','is_reassigned','questionnaire','vendor_feedback','negative_status'],
        }
    elif table == 'spot_checks':
        return {
            'select': f'''{alias}.id, {alias}.case_id, {alias}.check_status,
                         {alias}.place_of_accident, {alias}.police_station, {alias}.district,
                         {alias}.fir_number, {alias}.time_of_accident, {alias}.accident_brief,
                         {alias}.triggers, {alias}.vendor_evidence AS evidence,
                         {alias}.vendor_documents AS vendor_documents, {alias}.case_documents AS case_documents,
                         {_first_existing_select_expr(table, alias, ['case_manager_feedback', 'admin_feedback'])} AS case_manager_feedback,
                         {_select_expr(table, alias, 'is_reassigned', 'FALSE')} AS is_reassigned,
                         {_select_expr(table, alias, 'questionnaire', "'{{}}'::jsonb")} AS questionnaire,
        ...
        return {
            'select': f'''{alias}.id, {alias}.case_id, {alias}.check_status,
                         {alias}.court_name, {alias}.fir_number, {alias}.mv_act,
                         {alias}.fir_delay_days, {alias}.bsn_section, {alias}.ipc,
                         {_select_expr(table, alias, 'statement')} AS statement,
                         {_select_expr(table, alias, 'triggers')} AS triggers,
                         {alias}.vendor_evidence AS evidence,
                         {alias}.vendor_documents AS vendor_documents, {alias}.case_documents AS case_documents,
                         {_first_existing_select_expr(table, alias, ['case_manager_feedback', 'admin_feedback'])} AS case_manager_feedback,
                         {_select_expr(table, alias, 'is_reassigned', 'FALSE')} AS is_reassigned,
                         {_select_expr(table, alias, 'questionnaire', "'{{}}'::jsonb")} AS questionnaire,
            ...
            'fields': ['id','case_id','check_status','court_name','fir_number','mv_act',
                        'fir_delay_days','bsn_section','ipc','statement','triggers','evidence',
                        'vendor_documents','case_documents','case_manager_feedback','is_reassigned',
                        'questionnaire','vendor_feedback','negative_status'],
        }
    elif table == 'rti_checks':
        return {
            'select': f'''{alias}.id, {alias}.case_id, {alias}.check_status,
                         {_select_expr(table, alias, 'statement')} AS statement,
                         {_select_expr(table, alias, 'triggers')} AS triggers,
...
def get_vendor_check_detail(request: HttpRequest, case_id: int, check_type: str):
    ...
    try:
        with connections['default'].cursor() as cursor:
            ...
            if not case_row:
                # Fallback to insurance_case for newly created ORM cases
                cursor.execute(f"""
                    SELECT id, claim_number, 
                           {_select_expr('insurance_case', 'ic', 'client_name', 'NULL')} AS client_name,
                           category,
                           {_select_expr('insurance_case', 'ic', 'case_receive_date', 'NULL')} AS case_receive_date,
                           {_select_expr('insurance_case', 'ic', 'case_due_date', 'NULL')} AS case_due_date,
                           {_select_expr('insurance_case', 'ic', 'tat_days', 'NULL')} AS tat_days,
                           {_first_existing_select_expr('insurance_case', 'ic', ['sla', 'sla_status'])} AS sla,
                           {_select_expr('insurance_case', 'ic', 'case_type', 'NULL')} AS case_type,
                           {_select_expr('insurance_case', 'ic', 'full_case_status', 'NULL')} AS full_case_status,
                           {_select_expr('insurance_case', 'ic', 'scope_of_work', 'NULL')} AS scope_of_work,
                           {_select_expr('insurance_case', 'ic', 'investigation_report_status', 'NULL')} AS investigation_report_status
                    FROM insurance_case ic WHERE id = %s
                """, [case_id])
                case_row = cursor.fetchone()
