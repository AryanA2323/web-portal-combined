...
def _table_exists(table_name: str) -> bool:
    """Return whether a table exists in the current database schema."""
    ...
    try:
        ...
        logger.warning(f"Failed to inspect table {table_name}: {exc}")
        exists = False
...
def _optional_column_sql(table_name: str, alias: str, column_name: str, default_sql: str = "''") -> str:
    """Return aliased column SQL, or a default SQL expression if the column is absent."""
    if _column_exists(table_name, column_name):
        return f"{alias}.{column_name}"
    return default_sql


def _case_list_order_clause(alias: str = "c") -> str:
    """Return a production-safe ORDER BY clause for the incident-db cases table."""
    if _column_exists("cases", "created_at"):
        return f"{alias}.created_at DESC NULLS LAST, {alias}.id DESC"
    if _column_exists("cases", "updated_at"):
        return f"{alias}.updated_at DESC NULLS LAST, {alias}.id DESC"
    if _column_exists("cases", "case_receive_date"):
        return f"{alias}.case_receive_date DESC NULLS LAST, {alias}.id DESC"
...
class AuditLogEntrySchema(Schema):
    """Audit log entry for caseManager portal."""
    event_time: datetime
    event_type: str
    actor: str
    description: str
...
def is_admin_or_super_admin(user) -> bool:
    """Check if user is caseManager or super admin."""
    return (
...
def get_cases_incident_db(
    ...
    try:
        with connections['default'].cursor() as cursor:
            ...
            offset = (page - 1) * page_size
            order_clause = _case_list_order_clause("c")
            cursor.execute(f"""
                SELECT
                    ROW_NUMBER() OVER (ORDER BY {order_clause}) AS seq_num,
                    c.*
                FROM cases c
                WHERE {where}
            ...
            # ─── claimant_checks ───────────────────────────────────────────
            claimant_income_expr = _optional_column_sql("claimant_checks", "cc", "claimant_income", "NULL")
            claimant_negative_status_expr = _optional_column_sql("claimant_checks", "cc", "negative_status")
            claimant_vendor_feedback_expr = _optional_column_sql("claimant_checks", "cc", "vendor_feedback")
            cursor.execute(f"""
                SELECT cc.case_id, cc.check_status,
                       cc.claimant_name  AS name,
                       cc.claimant_contact AS contact,
                       cc.claimant_address AS location,
                       CAST({claimant_income_expr} AS TEXT) AS key_info,
                       cc.statement,
                       cc.triggers AS triggers,
                       {claimant_negative_status_expr} AS negative_status,
                       {claimant_vendor_feedback_expr} AS vendor_feedback,
            ...
            for r in cursor.fetchall():
                ...
                if cid in checks_by_case:
                    checks_by_case[cid].append({
                        ...
                        "negative_status": r[8] or "",
                        "vendor_feedback": r[9] or "",
                        "assigned_vendor_id": r[10],
                        "assigned_vendor_name": r[11],
                    })

            # ─── insured_checks ────────────────────────────────────────────
            insured_cum_driver_expr = (
                "ic.insured_cum_driver"
                if _column_exists("insured_checks", "insured_cum_driver")
                else "FALSE"
            )
            insured_negative_status_expr = _optional_column_sql("insured_checks", "ic", "negative_status")
            insured_vendor_feedback_expr = _optional_column_sql("insured_checks", "ic", "vendor_feedback")
            cursor.execute(f"""
                SELECT ic.case_id, ic.check_status,
                       ...
                       {insured_negative_status_expr} AS negative_status,
                       {insured_vendor_feedback_expr} AS vendor_feedback,
                       {insured_cum_driver_expr} AS insured_cum_driver,
            ...
            for r in cursor.fetchall():
                ...
                if cid in checks_by_case:
                    ...
                    checks_by_case[cid].append({
                        "type": "Insured Check",
                        "check_status": "Not Initiated" if not r[14] else (r[1] or "WIP"),
                        "name": r[2] or "—",
                        "contact": r[3] or "—",
                        "location": r[4] or "—",
                        ...
                        "vendor_feedback": r[12] or "",
                        "insured_cum_driver": r[13],
                        "assigned_vendor_id": r[14],
                        "assigned_vendor_name": r[15],
                    })

            # ─── driver_checks ─────────────────────────────────────────────
            driver_cum_driver_expr = (
                "dc.insured_cum_driver"
                if _column_exists("driver_checks", "insured_cum_driver")
                else "FALSE"
            )
            driver_negative_status_expr = _optional_column_sql("driver_checks", "dc", "negative_status")
            driver_vendor_feedback_expr = _optional_column_sql("driver_checks", "dc", "vendor_feedback")
            cursor.execute(f"""
                SELECT dc.case_id, dc.check_status,
                       ...
                       {driver_negative_status_expr} AS negative_status,
                       {driver_vendor_feedback_expr} AS vendor_feedback,
                       {driver_cum_driver_expr} AS insured_cum_driver,
            ...
            for r in cursor.fetchall():
                ...
                if cid in checks_by_case:
                    ...
                    checks_by_case[cid].append({
                        "type": "Driver Check",
                        "check_status": "Not Initiated" if not r[13] else (r[1] or "WIP"),
                        "name": r[2] or "—",
                        "contact": r[3] or "—",
                        "location": r[4] or "—",
                        "key_info": dl_info,
                        ...
                        "vendor_feedback": r[11] or "",
                        "insured_cum_driver": r[12],
                        "assigned_vendor_id": r[13],
                        "assigned_vendor_name": r[14],
                    })

            # ─── spot_checks ───────────────────────────────────────────────
            spot_negative_status_expr = _optional_column_sql("spot_checks", "sc", "negative_status")
            spot_vendor_feedback_expr = _optional_column_sql("spot_checks", "sc", "vendor_feedback")
            cursor.execute(f"""
                SELECT sc.case_id, sc.check_status,
                       ...
                       sc.accident_brief,
                       sc.triggers,
                       {spot_negative_status_expr} AS negative_status,
                       {spot_vendor_feedback_expr} AS vendor_feedback,
            ...
            for r in cursor.fetchall():
                ...
                if cid in checks_by_case:
                    ...
                    checks_by_case[cid].append({
                        ...
                        "negative_status": r[9] or "",
                        "vendor_feedback": r[10] or "",
                        "assigned_vendor_id": r[11],
                        "assigned_vendor_name": r[12],
                    })

            # ─── chargesheets ──────────────────────────────────────────────
            chargesheet_bsn_expr = _optional_column_sql("chargesheets", "cs", "bsn_section")
            chargesheet_negative_status_expr = _optional_column_sql("chargesheets", "cs", "negative_status")
            chargesheet_vendor_feedback_expr = _optional_column_sql("chargesheets", "cs", "vendor_feedback")
            chargesheet_advocate_status_expr = _optional_column_sql("chargesheets", "cs", "advocate_status")
            cursor.execute(f"""
                SELECT cs.case_id, cs.check_status,
                       ...
                       cs.mv_act,
                       cs.fir_delay_days,
                       {chargesheet_bsn_expr} AS bsn_section,
                       cs.ipc,
                       cs.statement,
                       cs.triggers,
                       {chargesheet_negative_status_expr} AS negative_status,
                       {chargesheet_vendor_feedback_expr} AS vendor_feedback,
                       cs.assigned_vendor_id,
                       v.company_name AS assigned_vendor_name,
                       {chargesheet_advocate_status_expr} AS advocate_status
        ...
        return {"cases": [], "total": 0, "debug_error": str(exc)}
...
def _fetch_ai_brief_case_context(case_id: int) -> dict:
    ...
    """
    policy_doc_expr = _optional_column_sql("cases", "c", "policy_document", "NULL")
    petition_doc_expr = _optional_column_sql("cases", "c", "petition_document", "NULL")
    other_doc_expr = _optional_column_sql("cases", "c", "other_document", "NULL")
    ...
    with connections['default'].cursor() as cursor:
        cursor.execute(
            ...
            SELECT
                ...
                c.case_receive_date,
                c.category,
                {policy_doc_expr} AS policy_document,
                {petition_doc_expr} AS petition_document,
                {other_doc_expr} AS other_document,
...
def generate_ai_brief_report(
    ...
    if not is_admin_or_super_admin(request.user):
        raise HttpError(403, "CaseManager access required")
...
@router.delete(
    ...
    summary="Delete Case",
    description="Delete a case and all its related verification checks. CaseManager access required.",
)
...
@router.patch(
    ...
    description="Update the full_case_status of a case. CaseManager access required.",
)
...
def upload_case_documents(
    ...
    if not is_admin_or_super_admin(request.user):
        return 403, {"error": "CaseManager access required"}
    ...
    try:
        ...
        if policy_files:
            ...
            if _column_exists("cases", "policy_document"):
                updates.append("policy_document = %s")
                params.append(saved_urls[0] if len(saved_urls) == 1 else json.dumps(saved_urls))
            else:
                logger.warning("Skipping policy document column update because cases.policy_document is missing")
        ...
        if petition_files:
            ...
            for f in petition_files:
                ...
                saved_path = default_storage.save(file_path, ContentFile(f.read()))
                saved_urls.append(default_storage.url(saved_path))
            if _column_exists("cases", "petition_document"):
                updates.append("petition_document = %s")
                params.append(saved_urls[0] if len(saved_urls) == 1 else json.dumps(saved_urls))
            else:
                logger.warning("Skipping petition document column update because cases.petition_document is missing")
        ...
        if other_files:
            ...
            for f in other_files:
                ...
                saved_path = default_storage.save(file_path, ContentFile(f.read()))
                saved_urls.append(default_storage.url(saved_path))
            if _column_exists("cases", "other_document"):
                updates.append("other_document = %s")
                params.append(saved_urls[0] if len(saved_urls) == 1 else json.dumps(saved_urls))
            else:
                logger.warning("Skipping other document column update because cases.other_document is missing")
...
def create_case(request: HttpRequest, payload: CreateCaseSchema):
    ...
    try:
        ...
        # Kept for caseManager/ORM compatibility. Non-fatal if it fails.
        orm_case_id = None
        try:
...
def get_audit_logs(
    ...
    """Get aggregated audit logs for caseManager portal."""
    if not is_admin_or_super_admin(request.user):
        return []
...
def auto_assign_vendor(request: HttpRequest, case_id: int):
    ...
    if not is_admin_or_super_admin(request.user):
        return {
            "success": False,
            "error": "CaseManager access required",
            "message": "Only caseManager users can assign vendors."
        }
...
@router.get(
    ...
    summary="Get All Clients (CaseManager)",
    description="Get list of all clients with full details for caseManager management."
)
def list_all_clients(request):
    """Get all clients for caseManager management."""
    if not is_admin_or_super_admin(request.user):
        raise HttpError(403, "CaseManager access required")
...
@router.post(
    ...
    description="Create a new client. CaseManager access required."
)
...
@router.put(
    ...
    description="Update an existing client. CaseManager access required."
)
...
@router.delete(
    ...
    description="Delete a client. CaseManager access required."
)
...
def list_vendors_for_cases(request, check_type: Optional[str] = None):
    ...
    if check_type == 'chargesheet':
        vendors = vendors.filter(user__role='ADVOCATE')
    else:
        vendors = vendors.filter(user__role='VENDOR')
        
    vendors = vendors.order_by('company_name')
...
@router.patch(
    ...
    description="Update assigned_vendor_id on a specific check row. CaseManager access required.",
    tags=["Cases"],
)
...
class CaseManagerReviewCheckSchema(Schema):
...
def review_check(request: HttpRequest, case_id: int, check_type: str, payload: CaseManagerReviewCheckSchema):
    from users.models import CustomUser
    if request.user.role not in [CustomUser.Role.CASE_MANAGER, CustomUser.Role.SUPER_ADMIN]:
        raise HttpError(403, "Only caseManagers can review checks")
    ...
    try:
        with connections['default'].cursor() as cursor:
            ...
            if table in ('insured_checks', 'driver_checks'):
                cursor.execute(f"SELECT insured_cum_driver FROM {table} WHERE case_id = %s", [case_id])
                row = cursor.fetchone()
                if row and row[0]:
                    cum_flag = True
            ...
            if action == 'accept':
                ...
                if cum_flag:
                    cursor.execute(f"UPDATE {twin_table} SET check_status = 'Verified' WHERE case_id = %s", [case_id])
                return {"success": True, "message": "Check accepted"}
                
            elif action == 'reject':
                base_feedback = payload.feedback or ''
                caseManager_name = f"{request.user.first_name} {request.user.last_name}".strip() or request.user.username
                feedback = f"{base_feedback} (Rejected by {caseManager_name})" if base_feedback else f"Rejected by {caseManager_name}"
                new_vendor_id = payload.new_vendor_id
                
                def get_update_sql(target):
                    set_clauses = ["check_status = 'Reassigned'"]
                    p = []
                    if _column_exists(target, 'case_manager_feedback'):
                        set_clauses.append("case_manager_feedback = %s")
                        p.append(feedback)
                    if _column_exists(target, 'is_reassigned'):
                        set_clauses.append("is_reassigned = TRUE")
                    if new_vendor_id:
                        set_clauses.append("assigned_vendor_id = %s")
                        p.append(new_vendor_id)
                    sql = f"UPDATE {target} SET {', '.join(set_clauses)} WHERE case_id = %s"
                    p.append(case_id)
                    return sql, p

                primary_sql, primary_params = get_update_sql(table)
                cursor.execute(primary_sql, primary_params)
                
                if cum_flag:
                    twin_sql, twin_params = get_update_sql(twin_table)
                    cursor.execute(twin_sql, twin_params)
...
def upload_check_media(
    ...
    if request.user.role not in [CustomUser.Role.CASE_MANAGER, CustomUser.Role.SUPER_ADMIN, CustomUser.Role.VENDOR]:
        raise HttpError(403, "CaseManager or vendor access required")
